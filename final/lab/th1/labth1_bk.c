...
#else
static float getBestDistance(uint32_t adc);
#endif
...
static void initTimer(void){
 	...
 	// clear OC1A/B when match set OC1A/B when BOTTOM. Set mode 14
...
static float getBestDistance(uint32_t adc){
	...
	int i;
	float minVal = 999; //initialize to be some big number
	float currDiff;
	int index = 0;

	for(i=0;i<REGISTERED_NUM;++i){

		currDiff = absDiff(lookupTable[i].adc,adc);
		if( minVal > currDiff){
			minVal = currDiff;
			index = i;
		}
